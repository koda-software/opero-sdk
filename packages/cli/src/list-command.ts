...
export abstract class ListCommand extends BaseCommand {
  ...
  protected async getList(path: string, flags: ListFlagValues & GlobalConfigFlags & OutputFormatFlags, extra: Query = {}): Promise<unknown> {
    const {settings} = await this.loadSettings(flags)
    const client = this.createApiClient(settings)
    const result = await client.get(path, {query: this.buildListQuery(flags, extra)})
